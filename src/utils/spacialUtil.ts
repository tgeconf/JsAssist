...
export const specialUtils = {
    ...
    isClosedShape(path: ICoord[]) {
        for (let i = 0, len = path.length; i < len - 1; i++) {
            ...
            for (let j = 0; j < len - 1; j++) {
                ...
                if (testP !== p && testNextP !== p && testP !== nextP && testNextP !== nextP
                    && specialUtils.isCross(testP, testNextP, p, nextP) && specialUtils.isCross(p, nextP, testP, testNextP)) {
                    return true;
                }
            }
        }
    ...
    pointInSector(p: ICoord, startAngle: number, endAngle: number, r: number) {
        ...
        var isInAngle = false, pointAngle = specialUtils.getPointAngle(p);
        if (endAngle < 360) {
        ...
        if (isInAngle) {
            var len = specialUtils.pointDist(p, { x: 0, y: 0 });
            isInAngle = r > len;
        }
        return isInAngle;
    },
